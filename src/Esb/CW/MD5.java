...
public class MD5 {
    ...
    public Map execute(Map param) throws Exception {
        ...
        log.error("财务_用款审批----->begin");
        RecordSet rsquery = new RecordSet();
        //响应对象
        JSONObject jsonObj_result = new JSONObject();

        String requestId = Util.null2String(param.get("RequestId"));//流程id requestid
        String querysql = "select * from formtable_main_128 where requestId=" + requestId;
        //用款审批数据
        Map UseList = new JSONObject();
        String datastr = "";
        rsquery.execute(querysql);
        while (rsquery.next()) {
            String sqsj = Util.null2String(rsquery.getString("sqrq") + ":00");
            /**ERP端付款唯一标识*/
            String erpInsId = Util.null2String(rsquery.getString("lcbh"));
            /**付款单位*/
            ...
            /**付款金额*/
            String amount = Util.null2String(rsquery.getString("spje"));
            /**是否自有资金付款*/
            String fundSource = "2";
            /**CNY-人民币 USD-美元 EUR-欧元 HKD-港元 GBP-英镑 参考资金系统主数据币种信息*/
            String currencyNo = Util.null2String(rsquery.getString("bzs"));
            switch (currencyNo){
                case "0":
                    currencyNo="CNY";
                    break;
            ...
            /**收款方账号*/
            String receAccountNo = Util.null2String(rsquery.getString("zh"));
            /**收款方户名*/
            String receAccountName = Util.null2String(rsquery.getString("fkdwmc"));
            /**收款方银行*/
            String receBankNo=Util.null2String(rsquery.getString("skyhbh"));
            /**支行号*/
            String receCnaps=Util.null2String(rsquery.getString("khhhh"));
            /**收款方开户行名称*/
            String receOpbankName = Util.null2String(rsquery.getString("khhmc"));
            /**对公\对私*/
            String persionFlag = Util.null2String(rsquery.getString("dgds"));
            /**加急标识加急-1 不加急-0*/
            String urgentFlag = Util.null2String(rsquery.getString("sfjj"));
            /**付款摘要信息*/
            String purpose = Util.null2String(rsquery.getString("fkzy"));
            /**附言*/
            //String remark = Util.null2String(rsquery.getString("yt"));
            /**录入人*/
            String creator = Util.null2String("sqr");
            /**是否集团计划内 0：否 1：是*/
            String numberValue1 = Util.null2String(rsquery.getString("sfzjtjhn"));
        ...
        //jsonObj_result.put("output",output);
/*        }else{
            jsonObj_result.put("errMsg",Util.null2String(resultobj.get("msg")));
            jsonObj_result.put("code",resultCode);
