 ...
 */
public class SynDeliveryInfoCron extends BaseCronJob {
    private Log log = LogFactory.getLog(SynDeliveryInfoCron.class.getName());
    ...
    public void execute() {
        ...
        log.info("------------------------------同步送达方主数据------------------------------开始");
        RecordSet recordSet = new RecordSet();
        String sapurl= Prop.getPropValue("wanxiang","sap.sdf.url");
        String urn= Prop.getPropValue("wanxiang","sap.sdf.urn");
        ...
        try {
            ...
            if(result.getCode().equals("S")){
                ...
                for(Map<String,Object> map :items){
                    ...
                    String sql="select * from uf_sdfzsj where KUNNR ='"+KUNNR+"'";
            ...
            if("1".equals(logstatus)) {
                UF_SAP_PRO_LOG uf_sap_pro_log = new UF_SAP_PRO_LOG("同步送达方主数据", "1", soapXml, resultXml, result.toString(), "", "", "OA", "-1", result.getCode(), (System.currentTimeMillis() - startTime) + "ms", InetAddress.getLocalHost().getHostAddress(), sapurl);
                ModeDataUtil.SaveSapProLogInfo(uf_sap_pro_log);
            }
        }catch (Exception e) {
            e.printStackTrace();
        }
        log.info("------------------------------同步送达方主数据------------------------------结束");
    }
