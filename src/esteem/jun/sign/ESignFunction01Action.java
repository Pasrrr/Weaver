 ...
 */
public class ESignFunction01Action extends BaseBean implements Action {
    private Log log = LogFactory.getLog(ESignFunction01Action.class.getName());
    ...
    public String execute(RequestInfo requestInfo) {
        ...
        try {
            ...
            if(ejkzt.equals(status_flag.get("bs0"))){
                ...
                String qswjfj  = Util.null2String(baseActionInfo.getMainTableValue("xgzwjgl"));
                if("".equals(qswjfj)){
                    qswjfj = baseActionInfo.getMainTableValue("fj");
                }
                Map<String,String> fileInfo =ESigTreasureUtil.getFileInfoByDocAttachment(qswjfj,user);
            ...
            // 文件准备完成  开始已创建流程
            if(ejkzt.equals(status_flag.get("bs2"))){
                log.info("############################################创建签署流程############################################begin");
                //签署文件流程
                CreateFlowByFilesRequest createFlowByFilesRequest =createFlowByFilesRequest(tableName,requestId);
    ...
    public CreateFlowByFilesRequest createFlowByFilesRequest(String tableName,String requestId){
        RecordSet recordSet = new RecordSet();
